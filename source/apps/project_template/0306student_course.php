...
<body>
   ...
   var view_url = 'alertview.json?nocache=' + (new Date()).getTime();
   //////////////////////
      ...
      $.getJSON(url, function(data) {
         ...
         for (var i in data.feature) {
            ...
            marker = L.marker([data.feature[i].lat, data.feature[i].lng], {
               UNIID,
               icon: icon1
            }).addTo(layer).on('click', editMarker2);
...
//            console.log(UNIID);
   ...
   $(function() {
      swal("Welcome!", "Hint : Right-click to add temporary marker !", "success");
      //$(document).ready(function(){
      var magni;
      ...
      $. getJSON("tile_config.json", function(tile_config){
         ...
         function ForViewchangeFocal(e,zoom,zoom_now,magni,lat,lng,tile_config) {
            ...
            for (var i in tile_config){
               ...
               myMap.setView([lat,lng],zoom_now)
               myMap.addLayer(drawnItems);
               //myMap.addLayer(YoloV4);
         ...
         var alert_view = L.easyButton(
            '<img src="./alert.svg" style="left: -2px;position: absolute;top: 2px;width: 18px;">',
            function(){
               console.log("123");
               getviewData();
            }).addTo(myMap);
   ...
   */
         ...
         editMarker2 = function(a) {
            console.log(this);
            $('#lecture').show();
            //document.getElementById("lec").innerHTML='<iframe src="./generate_html/'+ a +'" frameborder="1" class=" " ;></iframe>';
            $.ajax({
               url: "student_editor.php",
               data: "uid=" + this.options.UNIID,
               datatype: "text",
               type: "POST",
               success: function(data) {
                  $("#editor").html(data);
         ...
         myMap.on("contextmenu", function(event) {
            ...
            var popup = marker.bindPopup(
               '</br><button class="btn btn-secondary" onclick="deleteMarker(\'' + uniqueID + /|/ + id +
               '\')">Delete Marker</button>' +
               '<button class="btn btn-secondary" onclick="editMarker(\'' + uniqueID +
               '\')">Edit Marker</button>');
         ...
         */
         ...
         if(data.magni==40){
            zoom=4;
         }else if(data.magni==100){
            zoom=5;
            console.log(zoom)
         }else if(data.magni==200){
            zoom=6;
            console.log(zoom)
         }
         else if(data.magni==400){
            zoom=7;
            console.log(zoom)
         }
         ForViewchangeFocal(data.focal,zoom,data.zoom,data.magni,data.lat,data.lng,tile_config)
         });
      };
         ...
         myMap.on('draw:edited', function(e) { //拖移改變圖標位置
            ...
            layers.eachLayer(function(layer) {
               deleteMarker(layer.options.UNIID)
               var data = {
